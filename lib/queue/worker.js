...
function Worker(queue, type, base, range) {
  this.queue = queue;
  this.type = type;
  this.base = base;
  this.range = range;
...
Worker.prototype.start = function(fn){
  ...
  self.getJob(function(err, job){
    ...
    if(self.base) {
      if(job.id % self.base !== self.range) {
        job.inactive();
        return setTimeout(function(){ self.start(fn); }, self.interval);        
      }
    }
    self.process(job, fn);
  });
...
Worker.prototype.zpop = function(key, fn){
  ...
  var script = "local val = redis.call('zrange', KEYS[1], 0, 0)" + "\n" +
         "if val then redis.call('zremrangebyrank', KEYS[1], 0, 0) end" + "\n" +
         "return val" + "\n";

  this.client.eval(script, 1, key, function(err, res){
      if (err) return fn(err);
      var id = res[0];
      fn(null, id);
    }); 
