    ...
    ids.forEach(function(id){
      exports.get(id, function(err, job){
        ...
        if (!job) {
          ...
          getSearch().remove(id);
          client.del('q:job:' + id, noop);
          client.zrem('q:jobs', id);
          client.zrem('q:jobs:' + 'failed', id);
          client.zrem('q:jobs:' + 'inactive', id);
          client.zrem('q:jobs:' + 'complete', id);
          client.zrem('q:jobs:' + 'delayed', id);

          client.zrem('q:jobs:sync jobs:failed', id);
          client.zrem('q:jobs:sync jobs:complete', id);
          client.zrem('q:jobs:sync jobs:inactive', id);
          client.zrem('q:jobs:sync jobs:delayed', id);
          client.zrem('q:jobs:bttt jobs:failed', id);
          client.zrem('q:jobs:bttt jobs:complete', id);
          client.zrem('q:jobs:bttt jobs:inactive', id);
          client.zrem('q:jobs:bttt jobs:delayed', id);
          client.zrem('q:jobs:insights jobs:failed', id);
          client.zrem('q:jobs:insights jobs:complete', id);
          client.zrem('q:jobs:insights jobs:inactive', id);
          client.zrem('q:jobs:insights jobs:delayed', id);
          client.zrem('q:jobs:tasks jobs:failed', id);
          client.zrem('q:jobs:tasks jobs:complete', id);
          client.zrem('q:jobs:tasks jobs:inactive', id);
          client.zrem('q:jobs:tasks jobs:delayed', id);
          console.log('removed', id);
          return fn(new Error('invalid job id'));
        }
 ...
 * Set state to `state`.
 *
 * If we are changing to 'inactive' we have to manipulate the
 * priority field in such way that kue always picks first
 * the jobs with the highest priority and among those jobs, the job
 * that has been queued longer.
 *
 * 
 * As kue considers the lowest the priority value, the highest priority, this implies:
 *
 *  - If the priority is normal or low (0 or 10), we take the current timestamp and concatenate it after a dot.
 *    This way, the priority will contain something like '(0-10).timestamp'. The first job to become inactive will have
 *    a lower total value and because of this, it will be picked first.
 *
 *  - If the priority is any other value, those values will be < 0, for this, we take the infinitum value and substract
 *    the current timestamp, so that, the earlier the job becomes queued, the bigger this number will be and being the priority
 *    negative, this will create lower priority values for older jobs.
 *
 * 
 * @param {String} state
 * @return {Job} for chaining
...
Job.prototype.state = function(state){
  ...
  if(state === 'inactive') {
    if(this._priority >= 0) {
      if(this._priority.toString().split('.').length > 1) {
        this._priority = this._priority.toString().split('.')[0] + '.' + now.getTime();
      }else {
        this._priority = this._priority + '.' + now.getTime();
      }
    }else {
      if(this._priority.toString().split('.').length > 1) {
        this._priority = this._priority.toString().split('.')[0] + '.' + (infinitum - now);
      }else {
        this._priority = this._priority + '.' + (infinitum - now);
      }
    }
  }
